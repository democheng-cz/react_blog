...
import defaultImage from '@/assets/images/猫和老鼠.png';
...
const PageTable = (props: PageTablePropsType, ref: any) => {
	...
	const renderTableColumn = (title: string, text: any, record: any) => {
		switch (title) {
			case '头像':
				return <Image src={text} style={{ width: '50px', height: '50px', borderRadius: '5px' }} />;
			case '封面':
				return (
					<Image
						src={text || defaultImage}
						style={{ width: '50px', height: '50px', borderRadius: '5px' }}></Image>
				);
			case '文章信息':
				return (
			...
			case '状态':
				return (
					<>
						<Switch checked={!!text} onChange={checked => handleUpdateState(checked, record)} />
					</>
				);
			case '时间':
				return (
